...
function validateBody(body: unknown): { valid: boolean; error?: string; data?: ContactBody } {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return { valid: false, error: 'Please provide a valid email address.' }
  if (!phone) return { valid: false, error: 'Phone number is required.' }
  if (!serviceInterest) return { valid: false, error: 'Service interest is required.' }
  if (!message) return { valid: false, error: 'Message is required.' }

  return {
    valid: true,
    data: { fullName, email, phone, company: company || undefined, serviceInterest, projectType: projectType || undefined, budgetRange: budgetRange || undefined, timeline: timeline || undefined, message },
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validation.valid || !validation.data) {
      return NextResponse.json({ success: false, error: validation.error || 'Validation error.' }, { status: 400 })
    }
